...
def calc_PET_24():
    T_mean   = weather_data_avg_24h.temperature      # Mean temperature in degrees Celsius
    R_s      = weather_data_avg_24h.solar_radiation  # Solar radiation in MJ/m²/day
    u_2      = weather_data_avg_24h.wind_speed       # Wind speed at 2 meters in m/s
    ...
    p        = weather_data_avg_24h.pressure         # Atmospheric pressure in kPa
    ...
    e_a      = e_s * (weather_data_avg_24h.humidity/100)  # Actual vapor pressure in kPa
...
def calc_PET_sowing():
    T_mean   = weather_data_avg_sowing.temperature      # Mean temperature in degrees Celsius
    R_s      = weather_data_avg_sowing.solar_radiation  # Solar radiation in MJ/m²/day
    u_2      = weather_data_avg_sowing.wind_speed       # Wind speed at 2 meters in m/s
    DELTA    = 4098 * (0.6108 * math.exp((17.27 * T_mean) / (T_mean + 237.3))) / ((T_mean + 237.3) ** 2) # Slope of the saturation vapor pressure-temperature curve in kPa/°C
    p        = weather_data_avg_sowing.pressure         # Atmospheric pressure in kPa
    y        = (1.013**-3 * p) / 0.622               # Psychrometric constant in kPa/°C
    DT       = DELTA / (DELTA + y * (1 + 0.34 * u_2))# Slope of the vapor pressure-temperature curve in kPa/°C
    PT       = y/(DELTA+y*(1+0.34*u_2))              # Psychrometric constant in kPa/°C
    TT       = 900/(T_mean + 273.3)*u_2              # Temperature term in kPa/°C
    e_s      = 0.6108 * math.exp((17.27 * T_mean) / (T_mean + 237.3)) # Saturation vapor pressure in kPa
    e_a      = e_s * (weather_data_avg_sowing.humidity/100)  # Actual vapor pressure in kPa
    dr       = 1 + 0.033 * math.cos((2 * math.pi) / 365 * datetime.now().timetuple().tm_yday) # Inverse relative distance Earth-Sun
    delta    = 0.409 * math.sin((2 * math.pi) / 365 * datetime.now().timetuple().tm_yday - 1.39) # Solar declination in radians
    lat_rad  = math.radians(51.989)                  #phi
    w_s      = math.acos(-math.tan(lat_rad) * math.tan(delta)) # Sunset hour angle in radians
...
def calc_water_deficit_24():
    water_content = 0
    water_deficit = water_content + (ET_0 - weather_data_avg_24h.rain)
    return water_deficit

def calc_water_deficit_sowing():
    water_content = 0
    water_deficit = water_content + (ET_0 - weather_data_avg_sowing.rain)
    return water_deficit    
...
if __name__ == "__main__":
    ...
    print ("\n24H REPORT:")
    print ("Potential evapotranspiration:\t",calc_PET_24(), "mm/day")
    print ("Water deficit:\t\t\t", calc_water_deficit_24(), "mm/day\n")
    ...
    print ("Acumulative potential evapotranspiration:\t",calc_PET_sowing(), "mm/day")
    print ("Acumulative water deficit:\t\t\t", calc_water_deficit_sowing(), "mm/day\n")
